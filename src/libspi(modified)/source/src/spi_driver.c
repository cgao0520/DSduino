...
{
  enum card_spi_return_code ret_code = CARD_SPI_STATUS_BUSY;

  if ( !CARD_SPI_BUSY_TEST) {
    *read_byte = CARD_EEPDATA;
    ret_code = CARD_SPI_STATUS_OK;
  }
...
{
  while ( write_cardSPI( byte) != CARD_SPI_STATUS_OK);
