...
/* Modified by Vincent (c_gao) July 22,2014*/
...
//function from outside. Added by Vincent (c_gao)
...
void do_line(int x1, int y1, int x2, int y2, int clr); //together with rectangle
...
void serror(int error), get_exp(int* result), putback();
...
#define STOP_CHAR	NULL//'\0'
unsigned char recv_buff[MAX_RECV_SIZE];

int basic_main(char* argv/*int argc, char** argv*/)
{
  char in[80];
  int answer;
  char *p_buf;
...
//  if(!load_program(p_buf,argv[1])) return 1;//exit(1);

  if(setjmp(e_buf)) return 1;//exit(1); /* initialize the long jump buffer */
...
void serror(int error)
{
  static const char *e[]= {   
    ...
    "DELAY needs a number",	//14
    "SEND syntax error",	//15
    "PSET syntax error",	//16
    "LINE syntax error",	//17
    "CIRCLE syntax error",	//18
    "DWRITE syntax error",	//19
    "AWRITE syntax error",	//20
    "DREAD syntax error",	//21
    "AREAD syntax error"	//22
  }; 
  printf("%s\n", e[error]); 
...
int get_token()
{
  ...
  if(isalpha(*prog) || *prog=='?') { /* var or command */
    while(!isdelim(*prog)) *temp++=*prog++;
    token_type=STRING;
  }
...
//////////////////////////////////////////////
//following functions are added by Vincent (c_gao)
void exec_cls()
{
	do_cls();
}
...
void exec_send()
{
  ...
  if(tok==EOL || tok==FINISHED) 
  {
    ...
    if(len>256)
    	len = 256;
     do_send(send_str, recv_buff, len);
     //printf(recv_buff);
  }
  else serror(15); /* error is not , or ; */	
}
...
void exec_line()
{
	...
	if(*token == ',')
	{
		...
		if(toupper(*token) == 'B' && toupper(token[1]) == 'F')
		{
			for(y=y1; y<=y2; y++)
				do_line(x1,y,x2,y,clr);
		}
		else if(toupper(*token) == 'B')
		{
			do_line(x1,y1,x2,y1,clr);
			do_line(x1,y2,x2,y2,clr);
			do_line(x1,y1,x1,y2,clr);
			do_line(x2,y1,x2,y2,clr);
		}		
		else
			serror(17);
	}
	else
		do_line(x1,y1,x2,y2,clr);
}
...
void exec_dread()
{
  ...
  get_token(); 
  var = toupper(*token)-'A'; /* get the var to save data*/

	data = do_dread(pin);

  variables[var] = data; /* store it */
}
